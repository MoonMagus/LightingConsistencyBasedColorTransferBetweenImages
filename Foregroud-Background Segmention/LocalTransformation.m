function LocalTransformation(Target, Source, OpenMatteT, OpenMatteS, MatteT, MatteS)
%%-----------------------------------------------------------------------
% 对图像对的蒙版区域进行局部变换:
%     Target : 目标图像.
%     MatteT : 目标图像蒙版.
...
%%------------------------------------------------------------------------
if nargin == 0
    Target = 'Images/transformers.jpg';
    MatteT = 'Images/transformersMatte.jpg';
    Source = 'Images/interview.jpg';
...
LABSourceLuminanceOnlyChannelA = LABSourceMatteChannelA;
LABSourceLuminanceOnlyChannelB = LABSourceMatteChannelB;
...
%% 合成最后结果.
LuminaOnlyResult = Lab2RGB(LABSourceMatteChannelL,LABSourceLuminanceOnlyChannelA,LABSourceLuminanceOnlyChannelB);
Whole = Lab2RGB(LABSourceChannelL,LABSourceChannelA,LABSourceChannelB);
HueSynResult = Lab2RGB(LABSourceMatteChannelL, LABSourceMatteChannelA, LABSourceMatteChannelB);
figure,imshow(Whole);
figure,imshow(LuminaOnlyResult);
figure,imshow(HueSynResult);
